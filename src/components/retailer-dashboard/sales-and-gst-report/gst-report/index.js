...
const GSTReport = () => {
  const { gst_report_table_data } = useSelector(
    (state) => state.ComponentPropsManagement
  );

  console.log("gst_report_table_data", gst_report_table_data);
  const columns = [
    {
      name: "HSN Code",
  ...
  return (
    <>
      <Card className="my-3">
        <CardBody>
          {`GST Report | Last 30 Days (Business Date: ${moment(
            new Date()
          ).format("DD-MMM-Y")})`}
        </CardBody>
      </Card>
      <DataTable
        ...
        // title={`GST Report | Last 30 Days (Business Date: ${moment(new Date()).format("DD-MMM-Y")})`}
        actions={actionsMemo}
      />
    </>
  );
