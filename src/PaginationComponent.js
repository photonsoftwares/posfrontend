...
const renderData = (filterdetails) => {
...
const PaginationComponent = ({ filterdetails }) => {
  ...
  const pages = [];
  for (let i = 1; i <= Math.ceil(filterdetails.length / itemsPerPage); i++) {
    pages.push(i);
  }
  ...
  const currentItems = filterdetails.slice(indexOfFirstItem, indexOfLastItem);
